...
- **FIFO render cache** — pages and client bundles are cached in-memory (capacity 1024 dev / 512
  prod, TTL 5 min dev / 30 min prod), invalidated automatically on file change.
...
- **Tower-based middleware** — composable CORS, timing, logging, and custom headers via
  `ruvyxa.config.ts`.
- **Wasm plugin runtime** — sandboxed WebAssembly plugins powered by Wasmtime with hot-reload,
  configurable permissions, and execution limits.
- **Parallel production bundling** — page client bundles are emitted concurrently and written back
  in deterministic route order.
...
- **Secure server actions** — validation hooks, origin checks, Fetch Metadata guards, a 64 KB body
  limit, and rate limiting are built in.
...
For a fuller integration app with dynamic routes, API routes, loaders, and server actions, see
...
middleware. Server actions additionally include built-in rate limiting, origin checks, and Fetch
Metadata guards.
