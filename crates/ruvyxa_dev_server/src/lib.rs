...
fn start_watcher(
    ...
    let mut watcher =
        notify::recommended_watcher(move |event: notify::Result<notify::Event>| match event {
            Ok(event) => {
                let _ = reload_tx.send(format!("{:?}", event.paths));
            }
...
async fn client_bundle(
    ...
    match render_client_bundle(&state.config, &query.path) {
        Ok(script) => {
...
pub fn render_request(config: &ServerConfig, request_path: &str, method: &str) -> Result<Response> {
    ...
    let Some(route_match) = find_route(&manifest, request_path) else {
        return Ok((StatusCode::NOT_FOUND, Html(error_page("Route not found"))).into_response());
    };
...
fn render_page(
    ...
    let client_script = client_hydration_script(request_path);
    let head_content = format!("{asset_links}<style>{styles}</style>");
...
fn client_hydration_script(request_path: &str) -> String {
    format!(
        r#"<script type="module" src="/__ruvyxa/client?path={}"></script>"#,
