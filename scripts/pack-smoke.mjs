...
for (const file of readdirSync(destination).filter((name) => name.endsWith(".tgz"))) {
  ...
  const packageJson = JSON.parse(execSync(`tar -xOf ${destination}/${file} package/package.json`).toString())
  ...
  if (packageJson.name === "ruvyxa") {
    ...
    assert(listing.includes(`package/native-bin/${platform}-${arch}/${executable}`), "ruvyxa package missing native binary")
  }

  if (packageJson.name === "create-ruvyxa") {
    assert(listing.includes("package/template/minimal/package.json"), "create-ruvyxa package missing template")
  }
}
