// GENERATED by scripts/sync-shared-runtime.mjs from @ruvyxa/core/framework-paths.
// Do not edit: edit packages/@ruvyxa/core/src/framework-paths.ts instead, then run
...
export function isReservedFrameworkPath(value) {
 ...
 */
export function isLiteralTransportPath(value) {
    return typeof value === 'string' && /^(\/[A-Za-z0-9._~-]+)+$/.test(value);
}
/** What a refused transport path is told it may contain. */
export const TRANSPORT_PATH_RULE = 'must be an exact absolute path of `/`-prefixed segments containing only letters, digits, `-`, `.`, `_`, or `~`';
/**
...
function transportBlock(value, key) {
...
function transportPath(value, key, fallback) {
    ...
    if (isReservedFrameworkPath(path)) {
        throw new TypeError(`RUV1602 config.${key}.path "${path}" collides with a reserved framework route.`);
    }
    return path;
}
function heartbeat(value, key) {
    ...
    if (typeof heartbeatMs !== 'number' ||
        ...
        throw new TypeError(`RUV1602 config.${key}.heartbeatMs must be an integer between ${HEARTBEAT_MIN_MS} and ${HEARTBEAT_MAX_MS}.`);
    }
    return heartbeatMs;
}
/**
 ...
 */
export function normalizeRealtimeConfig(value) {
    ...
    if (typeof capacity !== 'number' ||
        ...
        throw new TypeError(`RUV1602 config.realtime.capacity must be an integer between ${REALTIME_CAPACITY_MIN} and ${REALTIME_CAPACITY_MAX}.`);
    }
...
export function normalizeCollabConfig(value) {
...
//# sourceMappingURL=framework-paths.js.map