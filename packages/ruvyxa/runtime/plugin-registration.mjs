// GENERATED by scripts/sync-shared-runtime.mjs from @ruvyxa/core/plugin-registration.
// Do not edit: edit packages/@ruvyxa/core/src/plugin-registration.ts instead, then run
 ...
 */
export function isExactApplicationPath(value) {
...
export function normalizeRealtime(plugin, value) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw new TypeError(`plugin "${plugin}" native.claim('realtime@1') expects an options object`);
    }
    const options = value;
    const pathValue = (options.path ?? '/__ruvyxa/realtime');
    ...
    if (!Number.isInteger(heartbeatMs) ||
    ...
    if (isReservedFrameworkPath(pathValue)) {
        throw new TypeError(`plugin "${plugin}" realtime path "${pathValue}" collides with a reserved framework route`);
    }
...
export function normalizePresence(plugin, value) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw new TypeError(`plugin "${plugin}" native.claim('presence@1') expects an options object`);
    }
    const options = value;
    const pathValue = (options.path ?? '/__ruvyxa/collab');
    const heartbeatMs = (options.heartbeatMs ?? 25_000);
    if (!isLiteralTransportPath(pathValue)) {
    ...
    if (isReservedFrameworkPath(pathValue)) {
        throw new TypeError(`plugin "${plugin}" presence path "${pathValue}" collides with a reserved framework route`);
    }
