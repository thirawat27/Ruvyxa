...
/**
 * The content engine: native Markdown/MDX routes read as one content graph,
 * and a content API, search index, RSS feed, sitemap, and llms.txt derived
 * from it without duplicate metadata.
 *
 * Turned on by `content` in `ruvyxa.config.ts` and run by the project worker
 * (`packages/ruvyxa/runtime/project-worker.mjs`): `ruvyxa build` writes every
 * artifact under `assets/` through {@link ContentEngine.write}, and `ruvyxa
 * dev` answers the same paths live through {@link ContentEngine.artifact},
 * re-deriving them when a content page changes. Production servers serve the
 * built files, so the live path — which walks the whole content tree per
 * request to know whether its cache is current — never runs where crawlers
 * poll behind `no-cache`.
 */
...
export interface ContentEngineProjectConfig {
  appDir?: string
  site?: {
    url?: string
    title?: string
...
/** One generated file: what `/content.json` and its siblings answer with. */
...
export interface ContentEngine {
  /** The public paths the engine generates, in a stable order. */
  readonly paths: readonly string[]
  /** The `RUV2207` message when the project named no locale, else `undefined`. */
  readonly warning: string | undefined
  /** The live artifact at `pathname`, or `undefined` when there is none. */
  artifact(root: string, pathname: string): ContentArtifact | undefined
  /** Write every artifact into `<outDir>/assets`, the directory served as `/`. */
  write(root: string, outDir: string): void
}
...
/** Build a content engine from explicit options. */
export function contentEngine(options: ContentEngineOptions): ContentEngine {
  const normalized = normalizeContentEngineOptions(options)
  const paths: readonly string[] = Object.freeze([
  ...
  // Recursively walks the content tree and stats every page *to compute the
  // key its own cache is checked against*, so the syscalls are per request
  // whether the cache hits or not. Development only, by construction: the
  // build writes the same bytes once and production serves those.
  const liveArtifacts = (root: string): Map<string, ContentArtifact> => {
  ...
  const engine: ContentEngine = {
    paths,
    warning: unsetLocaleDiagnostic(
      normalized.configuredLocale,
      'content.engine.locale (or site.language)',
    ),
    artifact(root, pathname) {
      if (!paths.includes(pathname)) return undefined
      if (!isDirectory(path.resolve(root, normalized.appDir))) return undefined
      return liveArtifacts(root).get(pathname)
    },
    write(root, outDir) {
      for (const [outputPath, artifact] of createContentEngineArtifacts(root, normalized)) {
        writePublicAsset(outDir, outputPath, artifact.body)
      }
    },
  }
...
/**
 * The engine `content` in `ruvyxa.config.ts` declares, or `undefined` when the
 * config declines it. `content: true` takes every default; site identity comes
 * from the shared `site` block.
 */
export function createContentEngine(config: ContentEngineProjectConfig): ContentEngine | undefined {
...
function createContentEngineArtifacts(
  ...
  const feedBody = createRssFeed(
    { title: options.title, description: options.description, language: options.language },
    options.siteUrl,
    feedItems,
  )
