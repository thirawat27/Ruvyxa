...
แต่ละกรณีด้านล่างจะ build ผ่านแล้วไม่ทำอะไรเลย การค้นหา route จึงล้มเหลวแทน โดยแยกโค้ดกันคนละตัว
เพราะวิธีแก้ต่างกัน:

| ชุด                                              | เหตุผล                                                                         | โค้ด      |
| ------------------------------------------------ | ------------------------------------------------------------------------------ | --------- |
| page ที่เป็น `'use client'` + `serverComponents` | page ที่รันในเบราว์เซอร์ทั้งหมดไม่มีฝั่งเซิร์ฟเวอร์ให้เรนเดอร์                 | `RUV1011` |
| `export const ppr = true` + `serverComponents`   | partial pre-rendering สตรีม shell ผ่าน entry ที่ไปป์ไลน์นี้ไม่ได้สร้าง         | `RUV1019` |
| intercepting route + `serverComponents`          | การ intercept ถูกจับคู่จาก client route registry ที่ไปป์ไลน์นี้ไม่ได้ประกาศไว้ | `RUV1020` |
...
(มัน throw tagged signal) ฝั่ง server คือ `status(404, message?)` จาก `ruvyxa/server` ซึ่งคืน
`Response` สถานะ 404 ให้ handler ส่งกลับ — เดิมทั้งคู่ชื่อ `notFound` เหมือนกัน ตอนนี้แยกกันที่
import แทนที่จะต้องอ่านคำเตือน
...
- **route จริงต้องมีอยู่** เพราะ interception คือ overlay การ reload, การแชร์ลิงก์
  ...
  ล้มเหลวด้วย **RUV1006** ส่วน marker ที่ไต่ระดับมากกว่าที่ app มีจะล้มเหลวด้วย **RUV1018**
