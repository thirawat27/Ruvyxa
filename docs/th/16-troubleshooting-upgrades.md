...
| `RUV2205`                                     | plugin claim `realtime@1`/`presence@1` แต่ adapter ที่เลือกสร้าง build artifact ซึ่งถือ WebSocket ไว้ไม่ได้ | ให้บริการ deployment ด้วย `ruvyxa start` หรือยอมรับว่า endpoint นี้ไม่มีใน artifact ถ้าใช้ realtime เฉพาะตอน development |
