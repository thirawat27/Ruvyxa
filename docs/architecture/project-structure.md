...
- Do not move root folders (`crates`, `packages`, `tests`, `examples`, `templates`, `scripts`,
  `docs`) without a compatibility plan.
...
- Put Node runtime helpers under `packages/ruvyxa/runtime` and include them in
  `packages/ruvyxa/package.json#files`.
