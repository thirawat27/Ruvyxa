...
| Route-level code splitting                  | One hydration bundle per page route via `split: "route"`                          |
...
| Wasmtime plugin runtime                     | Sandboxed WebAssembly plugins with fuel, memory, and `allow` rules                |
| Configurable CORS, timing, `log` middleware | Tower-based composable middleware stack                                           |
