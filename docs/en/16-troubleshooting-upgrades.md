...
| `RUV2205`                                      | `realtime` or `collab` is on, and the selected adapter emits a build artifact, which cannot hold a WebSocket. | Serve the deployment with `ruvyxa start`, or accept that the endpoint is absent from the artifact when realtime is development-only. |
...
| `RUV1017`                                      | A catch-all folder has a child segment under it, which no URL can ever reach.                                 | Move the catch-all to the end of the route, or remove the segment beneath it.                                                        |
| `RUV1018`                                      | An intercepting marker climbs more URL levels than the route it sits in has.                                  | Use a shorter marker, or `(...)` to name the target from the app root.                                                               |
| `RUV1019`                                      | A page exports both `ppr` and `serverComponents`, which render through different entries.                     | Drop one of the two exports from the page.                                                                                           |
| `RUV1020`                                      | A `serverComponents` route carries an interception, whose client registry it never publishes.                 | Drop `serverComponents`, or move the interception to a route that does not use it.                                                   |
| `RUV1021`                                      | A directory under the app directory could not be read, so the routes below it could not be seen.              | Grant the build read access, or move the directory out of `app/`; a `_`-prefixed folder is not walked.                               |
| `RUV2208`                                      | `cache.handler` is configured, but the build output carries no build id to namespace its keys with.           | Run `ruvyxa build` again; an unprefixed key would be read by any other deployment sharing the store.                                 |
...
the nearest route boundary renders `not-found.tsx` — that is the one for a page.
`status(404, message?)` from `ruvyxa/server` returns a `Response` for an `app/api/` handler or a
loader to return. Before they were named apart both were `notFound`, so a page that imported the
server half rendered a `Response` object and an API route that imported the browser half threw
