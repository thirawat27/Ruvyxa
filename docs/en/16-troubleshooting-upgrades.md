...
| `RUV2205`                                      | A plugin claims `realtime@1`/`presence@1`, and the selected adapter emits a build artifact, which cannot hold a WebSocket. | Serve the deployment with `ruvyxa start`, or accept that the endpoint is absent from the artifact when realtime is development-only. |
