...
`plugin new` สร้างแพ็กเกจพร้อมเผยแพร่ที่ `plugins/request-logger/` โดยมี `src/index.ts`,
`package.json`, `tsconfig.json` และ `README.md` ให้รัน `pnpm build` เพื่อสร้าง `dist/` และ
`pnpm publish` เพื่อเผยแพร่เป็น npm library จากนั้นนำเข้า package ใน `ruvyxa.config.ts` แล้ว
ลงทะเบียนใน `config({ plugins: [...] })` ดู workflow เต็มได้ที่ [Plugins](plugins.md)
