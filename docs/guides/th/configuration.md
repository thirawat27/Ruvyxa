...
Plugin สร้างด้วย `definePlugin({ name, setup })` และลงทะเบียน middleware, `resolveId`, `transform`
หรือ `onBuildComplete` จาก `setup` เดียวกัน ทุก hook ทำงานตามลำดับที่ลงทะเบียนใน persistent plugin
runtime และ middleware ใช้ Fetch `Request`/`Response`
...
- `plugins`: array ของ plugin ที่สร้างด้วย `definePlugin` และมี `setup`
