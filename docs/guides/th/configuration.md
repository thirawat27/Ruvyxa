...
JavaScript build plugins ของ Ruvyxa โดยไม่เปลี่ยน Rust HTTP server ค่าเริ่มต้นคือ Node

```ts
export default config({
  runtime: 'bun', // 'node' (default) หรือ 'bun'
})
```

Node ยังเป็นค่าเริ่มต้น หากต้องการบังคับให้ Bun ถูกใช้ตั้งแต่การโหลด config ครั้งแรก ให้ตั้งค่า
bootstrap override ในคำสั่งของแอป: `RUVYXA_RUNTIME=bun bunx ruvyxa dev` ค่า override นี้เหมาะกับ CI
และมีลำดับความสำคัญเหนือ `runtime`
...
ปลั๊กอิน JavaScript จะทำงานผ่าน Node หรือ Bun worker แบบ persistent โดยค่าเริ่มต้น hook จะถูก
serialize เพื่อรักษา state เดิมของปลั๊กอิน หาก hook ทุกตัวที่ใช้ `resolveId` หรือ `transform` เป็น
